...
cell AMX_NATIVE_CALL rg_plant_bomb(AMX *amx, cell *params)
{
	...
	if (unlikely(params[arg_index] != 0))
	{
		CHECK_ISPLAYER(arg_index);
...
cell AMX_NATIVE_CALL rg_create_weaponbox(AMX* amx, cell* params)
{
	...
	CAmxArgs args(amx, params);
	CWeaponBox *pBox = g_ReGameFuncs->CreateWeaponBox(pItem, pPlayer, args[arg_modelname], args[arg_origin], args[arg_angles], args[arg_velocity], args[arg_lifetime], args[arg_packammo]);
...
cell AMX_NATIVE_CALL rg_death_notice(AMX* amx, cell* params)
{
	...
	CAmxArgs args(amx, params);
	CSGameRules->DeathNotice(pPlayer, params[arg_killer], params[arg_inflictor])
	return TRUE;
}
